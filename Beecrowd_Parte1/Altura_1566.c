...
int main()
{
    int NC, N, altura, first, alturas[231];  // Declaração de variáveis

    scanf("%d", &NC);  // Lê o número de casos de teste

    for (int k = 0; k < NC; ++k)  // Loop através de cada caso de teste
    {
        // Inicializa o array alturas com zeros
        for (int i = 0; i < 231; ++i) {
            alturas[i] = 0;
        }

        scanf("%d", &N);  // Lê o número de inteiros para o caso de teste atual

        // Conta a frequência de cada altura inteira
        for (int i = 0; i < N; ++i)
        {
            scanf("%d", &altura);  // Lê cada altura inteira
            alturas[altura]++;  // Incrementa a contagem para essa altura
        }

        first = 1;  // Flag para controlar o formato de impressão
        // Imprime as alturas ordenadas em ordem crescente
        for (int i = 20; i < 231; ++i)  // Itera sobre as alturas possíveis (de 20 a 230)
        {
            for (int j = 0; j < alturas[i]; ++j)  // Imprime cada altura alturas[i] vezes
            {
                if (first)
                    first = 0;  // Se for a primeira saída, evita imprimir espaço
                else
                    printf(" ");  // Imprime espaço entre as alturas
                printf("%d", i);  // Imprime a altura
            }
        }
        printf("\n");  // Avança para a próxima linha após imprimir as alturas para o caso de teste atual
    }

    return 0;  // Retorna 0 indicando término do programa
}
